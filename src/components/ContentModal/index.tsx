...
function ContentModal({ logo, logoAlt, projectName, icon, websiteLink, gitHubLink}: ContentModalProps) {
    ...
    return (
        <div className='flex flex-col h-full px-10 pt-7 pb-4 lg:px-7 lg:pt-5 lg:pb-3 sm:pt-4 xs:px-5 justify-center text-color18 xs:pt-3'>
            <div className="flex h-4/5 xs:flex-wrap">
                ...
                {websiteLink === '#' && gitHubLink !== '' ? (
                    <a href={websiteLink} className="w-1/3 md:w-[30%] outline-none flex flex-col items-center xs:w-full xs:mx-12 xs:my-4">
                        <img className="px-3 mt-1 mb-3 md:px-0 md:mb-2 md:mt-1 xs:mb-[0.38rem] max-h-[47px] max-w-[190px] md:max-w-[150px] md:max-h-[37px]" src={logo} alt={logoAlt} />
