...
function NavBar() {
    ...
    const NavCat = ({ icon, text, anchor, active }: NavCatProps) => (
        ...
        >
            ...
            >
                <i
                    className="group/icon rounded-full border border-white/10 relative flex items-center justify-center h-12 w-12 mt-2 mb-2 mx-auto transition-all duration-100 ease-linear cursor-pointer xs:h-11 xs:w-11 group-hover/link:transition-all group-hover/link:duration-100 group-hover/link:ease-linear minlg:group-hover/link:bg-darkBlue/10 group-[.active]/link:transition-all group-[.active]/link:duration-100 group-[.active]/link:ease-linear group-[.active]/link:bg-darkBlue/10"
                    style={anchor === '#contact' ? { paddingRight: '2px' } : {}}
                >
                    {icon}
                </i>
                <span className="hidden relative transition-all text-center cursor-pointer ml-[0.8rem] group-hover/menu:flex group-hover/menu:text-center group-hover/menu:sm:hidden group-hover/menu:md:hidden group-hover/menu:lg:hidden group-hover/link:font-medium group-[.active]/link:font-medium">
                    {text}
                </span>
            </a>
        </div>
