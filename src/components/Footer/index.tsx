...
function Footer() {
    ...
    const LinkFooter = ({ icon, text, link }: LinkFooterProps) => (
        <a
            href={link}
            target="_blank"
            className="flex flex-row items-center text-100 md:text-90 sm:text-85 xs:text-80 minlg:hover:text-white/70"
        >
    ...
    return (
        <footer
            id="contact"
            className="flex flex-col justify-center mx-24 lg:mx-[5%] pt-8 md:mx-[5%] sm:mx-6 xs:mx-4"
            itemScope
        ...
        >
            ...
            <div className="justify-between overflow-hidden px-0 py-10 lg:py-7 sm:py-7 xs:pt-6">
                <div
                    className="grid grid-cols-8 lg:grid-cols-3 mx-0 columns-3 -mb-6 pb-8 text-center justify-around lg:pb-5 md:pb-4 sm:flex sm:flex-wrap sm:justify-center sm:pb-3"
                    aria-label="Footer"
                >
                    <div className="flex col-span-3 lg:col-span-1 justify-around items-center pl-1 pr-2 lg:px-0 sm:order-2 sm:pl-8 xs:pl-4">
                        <Link
                            className="text-100 minlg:hover:text-white/70 text-center md:text-90 sm:text-85 xs:text-80"
                            to="/legalnotice"
                        >
                    ...
                    <div className="flex col-span-2 lg:col-span-1 justify-center gap-10 lg:gap-8 md:gap-4 sm:order-1 sm:w-full sm:px-[10%] sm:gap-20 sm:mx-0 sm:pb-4 xs:gap-14">
                        <LinkFooter
                            link={'https://github.com/MatiasLLL'}
                            icon={<FaGithub size="28" />}
                            text={'GitHub'}
                        />
                        <LinkFooter
                            link={'https://linkedin.com/in/matias-larralde'}
                ...
                <p className="mt-9 pb-2 text-center text-90 leading-5 md:text-85 sm:text-80 sm:mt-8 sm:pb-0 xs:text-75">
                    {t('footer.copyright')}
                </p>
            </div>
        </footer>
